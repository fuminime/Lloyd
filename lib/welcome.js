...
module.exports.welcome = async (iswel, isleft, XeonBotInc, anu) => {
  try {
    ...
    for (let num of participants) {
      ...
      try {
        pp_user = await XeonBotInc.profilePictureUrl(num, 'image');
      } catch {
      ...
      if (anu.action == 'add' && (iswel || setting.auto_welcomeMsg)) {
        ...
        if (isSetWelcome(anu.id, set_welcome_db)) {
          ...
          XeonBotInc.sendMessage(anu.id, { text: `${full_pesan}` });
        } else {
          var wc = await getBuffer(`https://telegra.ph/file/eaf642e46f00344e93a2c.png`);
          await XeonBotInc.relayMessage(anu.id, {
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          var full_pesan = (replace_pesan.replace(/@group/gi, groupName).replace(/@desc/gi, groupDesc));
          XeonBotInc.sendMessage(anu.id, { image: { url: pp_user }, mentions: [num], caption: `${full_pesan}` });
        } else {
...
module.exports.aDelete = async (setting, XeonBotInc, m) => {
...
✍️ *MessageType* : ${mek.mtype}
    `,
      ...
      await XeonBotInc.copyNForward(froms, mek, true);
      await delay(4000);
...
module.exports.oneTime = async (setting, XeonBotInc, m) => {
...
💬 *Caption* : ${m.msg.caption ? m.msg.caption : 'no caption'}`;
      ...
      await XeonBotInc.sendTextWithMentions(m.chat, teks, m);
      await delay(500);

      XeonBotInc.copyNForward(m.chat, true, {
        readViewOnce: true,
        quoted: m,
      });
    } catch (err) {
